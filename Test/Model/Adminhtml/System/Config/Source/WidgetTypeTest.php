...
use Comfino\ComfinoGateway\Model\Adminhtml\System\Config\Source\WidgetType;
...
/**
 * WidgetType delegates to SettingsManager::getWidgetTypesSelectList(), which calls the (final) SDK SettingsManager
 * singleton. The SDK class cannot be doubled, so a real instance is built through getInstance() with mocked PSR
 * collaborators, and its in-memory widgetTypesCache is pre-seeded so getWidgetTypes() returns deterministically
 * without any API call. getWidgetTypesSelectList() then maps the cache into the admin select-list shape.
 *
...
final class WidgetTypeTest extends TestCase
{
    ...
    /**
     * @param array<string, string> $widgetTypes
     */
    private function seedSdk(array $widgetTypes): void
    {
        ...
        $sdk = SdkSettingsManager::getInstance(
            $languageProvider,
        ...
        (new ReflectionProperty(SdkSettingsManager::class, 'widgetTypesCache'))->setValue($sdk, $widgetTypes);
    }

    public function testReturnsWidgetTypesFromSdkSettingsManager(): void
    {
        ConfigManagerHarness::install([]);

        $this->seedSdk(['textWidget' => 'Text widget', 'mixedWidget' => 'Mixed widget']);

        $this->assertSame(
            [
                ['value' => 'textWidget', 'label' => 'Text widget'],
                ['value' => 'mixedWidget', 'label' => 'Mixed widget'],
            ],
            (new WidgetType())->toOptionArray()
        );
    }

    public function testReturnsEmptySentinelWhenApiUnavailable(): void
    {
        /* Empty API key means getWidgetTypes() returns null, so the SDK emits the single empty-value sentinel which
           the Magento layer wraps in a translation. The required-entry validator then blocks saving. */
        ConfigManagerHarness::install([]);
        ...
        SdkSettingsManager::getInstance(
            $languageProvider,
            $this->createMock(ClientInterface::class),
            $this->createMock(RequestFactoryInterface::class),
            $this->createMock(StreamFactoryInterface::class),
        ...
        $options = (new WidgetType())->toOptionArray();
