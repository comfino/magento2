...
class ConfigManager
{
    ...
    /* Retry gate for a failed CETS access token claim. @see refreshErrorLoggingTokenIfNeeded() */
    private const ERROR_LOGGING_TOKEN_COOLDOWN_KEY = 'comfino_error_logging_token_claim_cooldown';
    private const ERROR_LOGGING_TOKEN_COOLDOWN = 900;
    ...
    /** @var array<int, ConfigurationManager> Store-scoped managers, keyed by store ID. */
    private static array $storeScopedManagers = [];
    ...
    /**
     * Returns a configuration manager bound explicitly to one store, bypassing ambient current-store resolution.
     *
     * {@see getInstance()} builds its StorageAdapter without a store ID, so every read resolves against whatever store
     * happens to be current — correct inside a store-bound HTTP request, wrong in the crontab/CLI areas, where the
     * ambient store is always the default one. Any code that must read a *specific* store's settings while running
     * outside that store's request (notably the outbound request queue drain, which resends operations enqueued by
     * other stores) has to go through here instead.
     *
     * The returned manager keeps its own in-memory value cache, isolated from the ambient one and from other stores,
     * via the SDK's per-scope instance map.
     */
    public static function getStoreScopedInstance(int $storeId): ConfigurationManager
    {
        if (!isset(self::$storeScopedManagers[$storeId])) {
            ...
            try {
                $manager->setDefaults(self::getDefaultConfigurationValues());
            } catch (LogicException $e) {
                /* Defaults are already registered on this scope's instance (it survived from an earlier call in this
                   request). Nothing to do - the values we need are present either way. */
            }

            self::$storeScopedManagers[$storeId] = $manager;
        }

        return self::$storeScopedManagers[$storeId];
    }

    /**
     * Store-scoped counterpart of {@see isSandboxMode()}. See {@see getStoreScopedInstance()} for when to use it.
     */
    public static function isSandboxModeForStore(int $storeId): bool
    {
        return (bool) self::getStoreScopedInstance($storeId)->getOptionWithDefault('COMFINO_IS_SANDBOX');
    }

    /**
     * Store-scoped counterpart of {@see getApiKey()}. See {@see getStoreScopedInstance()} for when to use it.
     */
    public static function getApiKeyForStore(int $storeId): ?string
    {
        $manager = self::getStoreScopedInstance($storeId);

        return self::isSandboxModeForStore($storeId)
            ? $manager->getOptionWithDefault('COMFINO_SANDBOX_API_KEY')
            : $manager->getOptionWithDefault('COMFINO_API_KEY');
    }
    ...
    /**
     * Claims a fresh CETS access token when the stored one is missing or about to expire.
     *
     * A failed claim installs a cooldown marker in the shared cache before returning. Without it every subsequent
     * request would retry the call, and with the default retry policy (3 attempts, 1/2/4 s connect escalation) an
     * unreachable API would add seconds of latency to every single request for as long as the outage lasts.
     */
    public static function refreshErrorLoggingTokenIfNeeded(): void
    {
        ...
        try {
            ...
            // CETS token claim is best-effort; back off so the failure is not retried on every request.
            self::startTokenClaimCooldown();
        }
    }
    ...
    /**
     * Returns true when a recent token claim failed and the retry cooldown has not elapsed yet. A cache backend failure
     * resolves to false, so an unavailable cache degrades to the previous retry-always behavior instead of suppressing
     * the claim indefinitely.
     */
    private static function tokenClaimOnCooldown(): bool
    {
        try {
            return CacheManager::getPool()->getItem(self::ERROR_LOGGING_TOKEN_COOLDOWN_KEY)->isHit();
        } catch (Throwable) {
            return false;
        }
    }

    private static function startTokenClaimCooldown(): void
    {
        try {
            $pool = CacheManager::getPool();
            $cooldownItem = $pool->getItem(self::ERROR_LOGGING_TOKEN_COOLDOWN_KEY);
            $cooldownItem->set(true);
            $cooldownItem->expiresAfter(self::ERROR_LOGGING_TOKEN_COOLDOWN);

            $pool->save($cooldownItem);
        } catch (Throwable) {
            // Cache unavailable - nothing to back off with.
        }
    }
}
